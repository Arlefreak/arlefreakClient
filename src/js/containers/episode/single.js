...
import SingleContainer from '../../components/single__episode__container';

const mapStateToProps = (state, ownProps) => {
  const { slug, episode_slug } = ownProps.match.params;
  const { apiCalls } = state;
  const { isFetching, lastUpdated, items: items } = apiCalls[
    'podcast/json/episodes/' + slug
  ] || {
    isFetching: true,
    items: [],
  };
  ...
  if (items.length > 0) {
    for (i; i < items.length; i++) {
      if (items[i].slug === episode_slug) {
        item = items[i];
        if (i > 0) prev = items[i - 1].slug;
        if (i < items.length - 1) next = items[i + 1].slug;
        break;
      }
    }
  }
...
const EpisodeSingle = connect(mapStateToProps, mapDispatchToProps)(
  SingleContainer
);
